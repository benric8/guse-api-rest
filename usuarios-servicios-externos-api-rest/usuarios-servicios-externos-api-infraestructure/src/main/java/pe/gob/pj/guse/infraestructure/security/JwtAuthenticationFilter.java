package pe.gob.pj.guse.infraestructure.security;
...
import pe.gob.pj.guse.domain.port.usecase.SeguridadUseCasePort;
import pe.gob.pj.guse.domain.utils.EncryptUtils;
...
import pe.gob.pj.guse.domain.utils.SecurityConstants;
	...
	*/
	...
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response,FilterChain filterChain, Authentication authentication) throws IOException {
		...
		String token = Jwts.builder()
				...
				.claim(ProjectConstants.CLAIM_USUARIO, user.getUsername())
				.claim(ProjectConstants.CLAIM_IP, request.getRemoteAddr())
				.claim(ProjectConstants.CLAIM_ACCESO, ProjectConstants.TOKEN_ACCESO_NEUTRO)
				.claim(ProjectConstants.CLAIM_LIMIT, ProjectUtils.sumarRestarSegundos(ahora, tiempoSegundosExpira + tiempoSegundosRefresh))
				.claim(ProjectConstants.CLAIM_NUMERO, 1)
				.compact();
